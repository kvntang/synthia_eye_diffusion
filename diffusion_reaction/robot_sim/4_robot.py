...
def main():
    ...
    while True:
        ...
        # Calculate how many seconds remain until the next capture
        time_remaining = int(capture_interval - (current_time - last_capture_time))
        if time_remaining < 0:
            time_remaining = 0

        # Check if it's time to capture and send the frame
        ...
        if stable_diff_img is not None:
            # 1) Make sure the two images have the same height before combining
            h1, w1 = frame.shape[:2]
            ...
            cv2.imshow("Webcam + Stable Diffusion", stable_diff_img_resized)
        else:
            # Create a blank white image with the same dimensions as the frame
            h1, w1 = frame.shape[:2]
            white_screen = np.ones((h1, w1, 3), dtype=np.uint8) * 255  # White image
            cv2.imshow("Webcam + Stable Diffusion", white_screen)
